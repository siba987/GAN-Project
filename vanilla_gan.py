...
def training_loop(train_dataloader, opts):
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            m = len(labels)
            D_real_loss = np.sum([(D(real_image)-1)**2] for real_image in real_images)
            ...
            D_fake_loss = np.sum([D(fake_image)**2] for fake_image in fake_images)
            ...
            G_loss =  np.sum([(D(fake_image)-1)**2] for fake_image in fake_images)
